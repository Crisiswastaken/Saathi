...
"""
The UI for the following features has been implemented and now only the functionality needs to be added. Go through the codebase, identify where the functionality for these features needs to be added and implement the functionality accordingly.
1. Document Upload: Implement the functionality for the document upload section in the settings drawer. This should allow users to upload documents from their device, which will then be stored locally on the device. The uploaded documents are being displayed appropriately, etc. Now the feature to be added is a toggle, ie a switch for each uploaded document that allows the user to select whether they want the AI to refer to that document during conversations or not. This toggle should be implemented and the state of the toggle should be stored locally on the device as well. Now when the toggle is on for a document, the AI should be able to refer to that document during conversations, and when the toggle is off, the AI should not refer to that document. Implement this functionality appropriately.
2. Report Viewer: Currently we have a template for the report, this report must be filled in with the appropriate data based on the conversations the user has with the AI. Now we need to implement the functionality for this. fields like Patient Summary, Symptoms, Medications, Allergies, Lifestyle notes, Recommendations, should be filled in by the AI via a toolcall only after a good amount of a conversation is complete and the AI has enough information to fill in these fields. The Vitals section should be filled in based on the details provided by the in app heart rate and spo2 detection tools, so you will need to implement the functionality to pull in the data from these tools and fill in the vitals section of the report accordingly. Most recent scan data should be pulled in and displayed in the vitals section. Implement this functionality appropriately, ensuring that the data is being pulled in correctly and the report is being filled in with the appropriate data based on the conversations and the vitals data. Medical History section should include the Include the uploaded documents that the user has provided in the uploaded section. Doctor notes and follow up plan can be left blank for now, we will fill these in the future when we have the functionality for the AI to provide recommendations and follow up plans based on the conversations and the data it has. 
